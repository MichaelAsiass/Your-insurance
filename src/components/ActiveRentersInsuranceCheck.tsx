...
interface RentersInsuranceProps {
  onSubmit: (hasRentersInsurance: boolean) => void;
}

const RentersInsurance: React.FC<RentersInsuranceProps> = ({ onSubmit }) => {
  ...
  const handleNextClick = () => {
    setIsLoading(true);
    setTimeout(() => {
      const hasRentersInsurance = response === 'Yes';
      onSubmit(hasRentersInsurance);
